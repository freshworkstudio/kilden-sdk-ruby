...
class ClientTrackTest < Minitest::Test
  ...
  def test_flush_chunks_batches_at_the_wire_limit
...
class ClientFlagsTest < Minitest::Test
  ...
  def test_flag_lookup_and_truthiness
    ...
    assert client.enabled?("on", "u1")
    # Cached: no extra request for the same distinct_id.
    refute client.enabled?("off", "u1")
    assert client.enabled?("exp", "u1")
  ...
  def test_unknown_flag_returns_default
    ...
    assert client.enabled?("ghost", "u1", default: true)
  ...
  def test_person_properties_bypass_the_cache
    ...
    assert client.enabled?("f", "u1")
    refute client.enabled?("f", "u1", person_properties: { "plan" => "free" })
  ...
  def test_failures_return_default_without_raising
    ...
    assert client.enabled?("f", "u2", default: true)

    @transport.respond_network_error

    refute client.enabled?("f", "u3")
  end

  def test_flag_input_validation
    client = build_client

    refute client.enabled?("", "u1")
